...
public class CameraFollow : MonoBehaviour
{
    ...
    [Tooltip("Position offset from parent")] [SerializeField]
    private Vector3 offset = default;

    [Tooltip("Position that the camera lerps towards each frame")]
    private Vector3 targetPosition;

    [Range(0.0F, 1.0F)] [SerializeField] private float lerpSpeed = 0.1f;
    ...
    void LateUpdate()
    {
        ...
        targetPosition = parent.transform.position +
                         (Quaternion.Euler(0.0f, transform.rotation.eulerAngles.y, 0.0f) * offset);

        RaycastHit hit;
        if (Physics.Raycast(parent.transform.position, targetPosition - parent.transform.position, out hit,
            maxDistanceToTarget))
        {
